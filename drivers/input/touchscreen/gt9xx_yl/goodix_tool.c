...
s32 init_wr_node(struct i2c_client *client)
{
	...
	while ((!cmd_data) && i)
	{
		cmd_data = kzalloc(i * DATA_LENGTH_UINT, GFP_KERNEL);
		if (NULL != cmd_data)
		{
			break;
		}
...
ssize_t goodix_tool_read(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	...
	else if (!cmd_head.wr)
	{
		...
		GTP_DEBUG("[CMD HEAD DATA] ADDR:0x%02x%02x.", cmd_data[0], cmd_data[1]);
		...
		while(data_len > 0)
		{
			...
			if (tool_i2c_read(cmd_data, len) <= 0)
			{
				GTP_ERROR("[READ]Read data failed!");
				return -EPERM;
			}

			ret = simple_read_from_buffer(&page[loc], size, ppos, &cmd_data[GTP_ADDR_LENGTH], len);
			if (ret < 0)
			{
				return ret;
			}
			...
			GTP_DEBUG_ARRAY(&cmd_data[GTP_ADDR_LENGTH], len);
